...
class RequestUser(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        print('跟踪看看2：', view_func.__name__ )

        app_weight = {'selfhelp':1,'appalarmset':3,'T1_supplierResponse':2,'Tao':4,'organ':5,'basedata':6,'syscfg':7,'workflow':8}
    ...
    def get_my_task(self,request):
        ...
        if request and request.user:
            query = TodoList.objects.filter(user=request.user.id,status=False)
            if query.count() == 0:
                return None
            else:
                return query.all()[:10]
