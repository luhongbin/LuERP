...
class code_apba(models.Model):
    sno = models.CharField('编码', max_length=20, null=True)
    gsb = models.CharField('公司',max_length=20,null=True)
    paybill = models.CharField('对账单号',max_length=30,null=True)
    lwmc = models.CharField('货物劳务名称',max_length=520,null=True)
    ggxh = models.CharField('规格     .',max_length=520,null=True)
    dw = models.CharField('计量单位',max_length=6,null=True)
    sl = models.DecimalField('数量',max_digits=14,decimal_places=2,null=True)
    cash = models.DecimalField('金额',max_digits=14,decimal_places=2,null=True)
    tax = models.DecimalField('税额',max_digits=14,decimal_places=2,null=True)
    totalcash = models.DecimalField('合计',max_digits=14,decimal_places=2,null=True)
    ...
    rjust_cash.short_description = '金额'

    def rjust_totalcash(self):
        return format_html(
            '<p align="right">{}</p>',
            self.totalcash,
        )
    rjust_totalcash.short_description = '合计'
    class Meta:
        managed = False
        db_table = 'code_apba'
...
class code_pmdnt200(models.Model):
    company = models.CharField('公司',max_length=20,null=True)
    jc = models.CharField('供应商',max_length=20,null=True)

    sno = models.CharField('编号',max_length=20,null=True)
...
class week_pmdnt200(models.Model):
    sno = models.CharField('编号',max_length=20,null=True)
    jc = models.CharField('供应商',max_length=20,null=True)
...
class apba_t(models.Model):
    STATUSapba_t = (
        (True, "结案"),
        (False, "对账中")
    )
    id = models.AutoField(id, primary_key=True)
    jhrq = models.DateField('进货日期', null=True)
    sno = models.CharField('供方编号', max_length=40, null=True)
    sname = models.CharField('供应商简称', max_length=40, null=True)
    paybill = models.CharField('对账单号', max_length=40, null=True)
    xc = models.CharField('项次', max_length=100, null=True)
    cgdh = models.CharField('采购单号', max_length=40, null=True)
    lwmc = models.CharField('货物劳务名称', max_length=500, null=True)
    ggxh = models.CharField('规格型号', max_length=500, null=True)

    sl = models.DecimalField('数量', max_digits=14, decimal_places=2, null=True)
    price = models.DecimalField('单价', max_digits=14, decimal_places=6, null=True)
    dw = models.CharField('单位', max_length=10, null=True)
    cash = models.DecimalField('金额', max_digits=14, decimal_places=2, null=True)
    tax = models.DecimalField('税额', max_digits=14, decimal_places=2, null=True)
    slv = models.DecimalField('税率', max_digits=14, decimal_places=3, null=True)
    totalcash = models.DecimalField('合计', max_digits=14, decimal_places=2, null=True)
    jhdh = models.CharField('进货单号', max_length=40, null=True)
    ddhm = models.CharField('订单号', max_length=40, null=True)
    note = models.CharField('备注', max_length=140, null=True)
    gsb = models.CharField('公司别', max_length=10, null=True)
    ok = models.BooleanField('对账完成状态',choices=STATUSapba_t)
...
class BO(models.Model):
    ...
    creator = models.CharField('创建者',blank=True,null=True,max_length=20)
    modifier = models.CharField('修改者',blank=True,null=True,max_length=20)
    creation = models.DateTimeField('创建时间',auto_now_add=True,blank=True,null=True)
    modification = models.DateTimeField('修改时间',auto_now=True,blank=True,null=True)
    mine = MineBOManager()
...
class BOAdmin(admin.ModelAdmin):
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        ...
        if app_info:
            try:
                ...
                todo_list = todo.model_class().objects.filter(inst=workflow_instance,status=0,user=request.user.id)
                x = todo_list.all()
                ...
                if workflow_instance.status == 3 and request.user.id == workflow_instance.starter:
                    can_restart = True
                    show_workflow_line = True
    ...
    def history_view(self, request, object_id, extra_context=None):
        ...
        app_info = get_app_model_info_from_request(request)
        # print app_info
        print('history_view APPINFO:',app_info)
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change:
            ...
            try:
                setattr(obj,'user',request.user.id)
            except Exception as e:
                pass
    ...
    export_selected_data.short_description = _("导出选择数据 %(verbose_name_plural)s")
